...
def test_apply_bandwidth_limit_calls_tc():
    with patch("app.network.tc._run") as mock_run:
        apply_bandwidth_limit("192.168.1.45", up_kbps=0, down_kbps=51200, wan_if="eth0")
        assert mock_run.call_count == 2  # class add + filter add for down only

def test_remove_bandwidth_limit_calls_tc():
    with patch("app.network.tc._run") as mock_run:
        remove_bandwidth_limit("192.168.1.45", up_kbps=0, wan_if="eth0")
        assert mock_run.call_count == 2  # filter del + class del for wan only
