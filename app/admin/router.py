...
def _verify_password(plain: str, hashed: str) -> bool:
    return _bcrypt.checkpw(
        plain.encode(), hashed.encode() if isinstance(hashed, str) else hashed
    )
...
async def dashboard_page(
    ...
    result = await db.execute(
        ...
        .order_by(Session.connected_at.desc())
        .limit(10)
    )
...
async def sessions_rows_fragment(
    request: Request,
    payload: dict = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db),
):
...
async def pms_page(
    ...
    return _templates.TemplateResponse(
        "pms.html",
        {
            ...
            "current_user": payload,
            "config": config,
            "adapter_type": adapter.type.value if adapter else None,
            "flash": flash,
        },
    )
...
async def check_pms_health(
    ...
    if not record or record.type == PMSAdapterType.standalone:
        ...
        try:
            config = (
                decrypt_config(record.config_encrypted)
            ...
            if not adapter_class:
                ...
                try:
                    ...
                    test_result = PMSTestResult(
                        ok=False, latency_ms=round(latency, 1), error=error_msg
                    )
        except Exception as e:
            error_msg = str(e) if str(e) else f"{type(e).__name__}"
...
async def list_mac_bypass(
    ...
    result = await db.execute(select(MacBypass).order_by(MacBypass.created_at.desc()))
...
async def assign_room_policy(
    room_id: uuid.UUID,
    body: RoomPolicyAssign,
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_superadmin),
):
...
async def analytics_data(
    ...
    if range_param not in RANGE_INTERVALS:
        return JSONResponse(
            status_code=400,
            content={"error": "invalid_range", "valid": list(RANGE_INTERVALS.keys())},
        )
    ...
    snaps_result = await db.execute(
        ...
        .order_by(UsageSnapshot.snapshot_at)
    )
    snaps = snaps_result.scalars().all()
    sessions_over_time = [
        {"timestamp": s.snapshot_at.isoformat(), "active_sessions": s.active_sessions}
        for s in snaps
    ]
    bandwidth_per_hour = [
        {
            "timestamp": s.snapshot_at.isoformat(),
            "bytes_up": s.total_bytes_up,
            "bytes_down": s.total_bytes_down,
        }
        for s in snaps
    ]
    ...
    peak_hours = [
        {"day_of_week": int(r.dow), "hour": int(r.hour), "count": int(r.count or 0)}
        for r in peak_result
    ]
    ...
    row = auth_result.one()
    auth_breakdown = {
        "room_auth": row.room_auth or 0,
        "voucher_auth": row.voucher_auth or 0,
    }
...
async def analytics_page(
    ...
    return _templates.TemplateResponse(
        "analytics.html",
        {
            "request": request,
            "current_user": payload,
            "analytics_data": analytics_data_val,
            "range": range_param,
            "flash": flash,
        },
    )
...
async def create_admin_user(
    ...
    try:
        ...
        raise HTTPException(
            422, {"error": "invalid_role", "allowed": ["staff", "superadmin"]}
        )
...
async def get_brand(
    ...
    return {
        ...
        "tc_text_th": b.tc_text_th,
        "tc_text_en": b.tc_text_en,
    ...
    for field, value in body.dict(exclude_none=True).items():
        if field == "language":
            try:
                ...
                raise HTTPException(
                    422, {"error": "invalid_language", "allowed": ["th", "en"]}
                )
...
async def upload_logo(
    ...
    if file.content_type not in ALLOWED_LOGO_MIME:
        raise HTTPException(
            422, {"error": "invalid_mime_type", "allowed": list(ALLOWED_LOGO_MIME)}
        )
    ...
    for field, value in body.dict(exclude_none=True).items():
        if field == "dns_mode":
            try:
                ...
                raise HTTPException(
                    422,
                    {"error": "invalid_dns_mode", "allowed": ["redirect", "forward"]},
                )
...
@router.get("/mac-bypass", response_class=HTMLResponse, include_in_schema=False)
async def mac_bypass_page(
    ...
    result = await db.execute(select(MacBypass).order_by(MacBypass.created_at.desc()))
    mac_bypass_list = result.scalars().all()
    flash = request.session.pop("flash", None)
    return _templates.TemplateResponse(
        "mac_bypass.html",
        {
            "request": request,
            "current_user": payload,
            "mac_bypass_list": mac_bypass_list,
            "flash": flash,
        },
    )
