...
async def start_https_redirect_server(portal_ip: str, portal_port: int) -> asyncio.Server:
    ...
    server = await asyncio.start_server(_handler, "0.0.0.0", HTTPS_REDIRECT_PORT, ssl=ssl_ctx)
    logger.info(f"HTTPS redirect server listening on :{HTTPS_REDIRECT_PORT} → {redirect_to}")
    return server
