...
### 2.11 MAC Address Bypass

**Menu:** MAC Bypass
**Role:** superadmin only

อุปกรณ์ที่อยู่ใน list นี้จะ**ข้ามการยืนยันตัวตน**และเข้าถึงอินเทอร์เน็ตได้ทันที

#### เพิ่ม MAC Address

1. กด **"Add MAC Address"**
2. กรอกข้อมูล:
   - **MAC Address** — รูปแบบ `AA:BB:CC:DD:EE:FF`
   - **Description** — คำอธิบาย (เช่น "Smart TV Living Room")
   - **Expires At** — วันหมดอายุ (optional)
3. กด **"Add"**
4. ระบบจะเพิ่ม MAC ใน nftables `mac_bypass` set
...
| Created | วันที่เพิ่ม |
| Expires | วันหมดอายุ (ถ้ามี) |
| Actions | ลบ |

#### Use Cases
- Smart TV, Chromecast, Alexa (IoT devices)
- Staff/management devices
- Special cases requiring no auth

---

### 2.12 Walled Garden Domains

**Menu:** Walled Garden
**Role:** superadmin only

แขกที่ยังไม่ได้ login สามารถเข้าถึง**domain ใน list นี้**ได้

#### เพิ่ม Domain

1. กด **"Add Domain"**
2. กรอกข้อมูล:
   - **Domain** — ชื่อ domain (เช่น `spotify.com`, `apple.com`)
   - **Description** — คำอธิบาย
3. กด **"Add"**
4. ระบบจะ resolve domain → IP และเพิ่มใน nftables `walled_garden` set

#### รายการ Walled Garden
...
#### Use Cases
- Streaming apps requiring activation (Spotify, Netflix)
- Hotel check-in pages
- Partner websites

---

### 2.13 Captive Portal API (RFC 8910/8908)

ระบบรองรับมาตรฐาน Captive Portal Detection ของระบบปฏิบัติการสมัยใหม่
...
| OS | Detection Method |
|----|------------------|
| Android | HTTP probe to `connectivitycheck.gstatic.com` |
| iOS/macOS | HTTP probe to `captiveportal.apple.com` |
| Windows 10/11 | DHCP Option 114 |
| Chrome OS | HTTP probe |
